...
class Authenticator extends AbstractFormLoginAuthenticator
{
    ...
    public function checkCredentials($credentials, UserInterface $user)
    {
        ...
        return (bool)password_verify($credentials['password'], $user->getPassword());
    }

    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey)
    {
        if ($token->getUser()->getActive()) {
            $currentRoleName = $token->getUser()->getCurrentRole()->getName();
            if($currentRoleName == "Pracownik" || $currentRoleName == "Manager")
                return new RedirectResponse($this->urlGenerator->generate('training_schedule_general'));
